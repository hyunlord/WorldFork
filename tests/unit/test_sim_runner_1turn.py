...
from service.game.state_v2 import Character, Race, WorldState
...
def test_sim_runner_run_single_turn_other_action_recognized() -> None:
    """WAIT 외 action — 인식되지만 후속 commit 처리."""
    actions = [
        PlayerAction(
    ...
    assert log.success  # 인식만 (★ 진짜 mutate는 후속 commit)
    assert "후속 commit" in log.message
...
def test_sim_runner_run_with_party_and_world() -> None:
    ...
    assert result.end_reason == "single_turn_2차_commit"
    assert result.final_hp_by_actor["비요른"] == 150
    assert result.final_hours_in_dungeon == 1
...
def test_sim_runner_run_empty_party() -> None:
    """빈 party."""
    runner = SimRunner(config=SimConfig())
    result = runner.run(party={}, world=_make_world())
