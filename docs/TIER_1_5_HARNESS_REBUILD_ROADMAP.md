...
### D5: Tier 1.5 마무리 + 자동 검증 정착 (★ 본인 #16 정공법)
...
D5 작업:
  - TaskContext.log_verify_attempt 파라미터 정리 (codex 지적 반영)
    success: bool 으로 변경 (score/verdict 제거 → 정보 격리)
  - Tier 1.5 총 회고 문서 작성
  - Eval Smoke 안정화 확인

⚠️ 사람 검증 시점 (★ 본인 #16):
  - play_w2_d5.py 실행 = 자동 검증만
  - 사람이 검증 = Tier 2 게이트 2+3 모두 통과 후
  - Web UI 거의 완성 + 게임 거의 완성 전에는 사람 검증 X

★ 게이트 1+1.5 통과 = 자동 검증 정착
★ 게이트 2+3 통과 = ★ 사람 검증 시작 (Tier 2 후반)
...
```
★ 본인 #16 핵심 정신:
  "사람 검증 = 거의 완성 가까이 되어야만"
  → Web UI 있어야 사람이 게임처럼 검증 가능
  → 파이썬 CLI 실행 = 사람 검증 X (자동 검증 도구)

Tier 1.5 D1-D5 완료 후:
  게이트 2: 게임 완성도 (Tier 2 전반)
    - 30턴 정상 완주
    - 풍부한 응답
    - 컨텐츠 다양화
    - 세계관 일관성 강화

  게이트 3: Web UI (Tier 2 중반)
    - Next.js 또는 FastAPI+HTML
    - 실제 게임처럼 브라우저에서 플레이
    - 점수/mech 시각화
    - UX 검증 가능한 UI

  ★ 게이트 2+3 모두 통과 = ★ 사람 검증 시작
  (본인 + 친구 검증, 브라우저에서 진짜 게임처럼)
