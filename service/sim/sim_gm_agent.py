...
A.6 commit base + F commit phase enforcement:
- A.6 (★ 8d7c04d): direct/dominance type rejection
- F commit (★ 본 commit): phase 본격 enforcement
  * DungeonPhase별 type 분포 강제
  * E commit 짚은 진짜 ceiling 직접 답
  * encounter spawn 빈도 본격 + 진행 단계 전환 본격

본 commit 본격 본질:
- 미궁 시간 → DungeonPhase 본격 결정
- phase별 권장 type 분포 prompt 동적 주입
- LLM 응답이 phase 본격 mismatch 시 retry
- A.6 + F 본격 통합 검증
"""
...
## 🔥 본격 진행 단계 (★ F commit, 작품 본문 정합)

미궁 시간에 따라 phase 본격 결정:

**PHASE_ENTRY (h<5)**: 진입 단계
- NARRATIVE 60% / ESSENCE 20% / EVENT 20%
- 어둠 속 가시거리 10m / 빛 활성 본질
- spawn 빈도 30% (★ 낮음)

**PHASE_EXPLORE (5≤h<24)**: 탐색 단계
- ESSENCE 35% / MONSTER 25% / ITEM 20% / EVENT 10% / NARRATIVE 10%
- 정수 흡수 본격 (★ 13/14화)
- 약한 몬스터 조우 시작 (★ 22화 노움)
- spawn 빈도 70% (★ 높음)

**PHASE_COMBAT (24≤h<72)**: 몬스터 단계
- MONSTER 40% / ESSENCE 25% / ITEM 15% / EVENT 10% / NARRATIVE 10%
- 본격 9등급 몬스터 조우 (★ 칼날늑대/레이스/위치스램프)
- 메시지 스톤 발견 (★ 11화)
- spawn 빈도 80% (★ 높음)

**PHASE_RIFT (h≥72)**: 균열 단계
- RIFT 40% / MONSTER 25% / EVENT 15% / NARRATIVE 10% / ESSENCE 10%
- 균열 4종 본격 spawn (★ 핏빛성채/빙하굴/녹색탄광/강철의 묘)
- 비석 공물 (★ 374화)
- spawn 빈도 90% (★ 매우 높음)

★ 본 turn phase 본격 user prompt에서 동적 명시.
★ phase 본격 X 정합 시 server가 응답 거부 (★ retry).
...
def _build_phase_section(hours_in_dungeon: int) -> str:
    """본 turn phase 본격 섹션 (★ F commit)."""
    phase = determine_phase(hours_in_dungeon)
    weights = PHASE_TYPE_WEIGHTS[phase]
    priority = PHASE_PRIORITY_TYPES[phase]

    lines = [
        f"**현재 phase**: {phase.value.upper()} (★ {hours_in_dungeon}h)",
        "",
        "**본 phase 권장 type 분포**:",
    ]
    for t in priority:
        w = weights.get(t, 0.0)
        lines.append(f"  - {t.value}: {w * 100:.0f}%")

    lines.extend(
        [
            "",
            "→ 본 turn은 위 분포 본격 정합 (★ phase mismatch 시 server 거부)",
        ]
    )
    return "\n".join(lines)


def _is_phase_mismatch(
    new_encounters: list[Encounter],
    phase: DungeonPhase,
) -> tuple[bool, str]:
    """본 turn encounter가 phase 본격 mismatch? (★ F commit Step C).

    검증:
    - non-narrative encounter의 type weight가 phase 권장에서
      PHASE_MISMATCH_WEIGHT_THRESHOLD 미만이면 mismatch
    - narrative-only는 항상 OK (★ A.6 mirror)
    ...
    weights = PHASE_TYPE_WEIGHTS[phase]
    for e in non_narrative:
        weight = weights.get(e.type, 0.0)
        if weight < PHASE_MISMATCH_WEIGHT_THRESHOLD:
            return (
                True,
                f"{e.type.value} phase {phase.value} X 정합 "
                f"(weight {weight:.0%})",
            )
...
def _build_gm_prompt(
    ...
    # ★ F commit 본격: phase section (★ 동적 주입)
    hours = int(world.get("hours_in_dungeon", 0))
    phase_section = _build_phase_section(hours)
    ...
    return (
        ...
        f"{forbidden_section}\n\n"
        f"## 이 턴 encounter 결정 (★ JSON 1개, phase 정합 + 다양 강제)\n"
    )
...
class SimGMAgent:
    """진짜 LLM SimGMAgent — server-side enforcement 본격 (★ A.6 + F).

    본 commit 본격 (★ F + A.6 통합):
    1. LLM 호출 → encounter 응답
    2. server-side rule check:
       - A.6: 직전 type / dominance
       - F: phase mismatch (★ 본 commit)
    3. 위반 시 retry (★ MAX_RETRY_COUNT)
    4. retry 모두 실패 시 fallback narrative
    5. enforcement_stats 본격 측정 (★ phase_mismatch 추가)
    """
    ...
    def __init__(
        ...
        # ★ F commit 본격: phase mismatch metric
        self._phase_mismatch_count = 0
    ...
    def enforcement_stats(self) -> dict[str, int]:
        """본 commit 본격 metrics (★ A.6 retry/fallback + F phase_mismatch)."""
        return {
            "retry_count": self._retry_count_total,
            "fallback_count": self._fallback_count,
            "phase_mismatch_count": self._phase_mismatch_count,
        }
    ...
    def generate_encounters(
        ...
        """LLM 호출 + server-side enforcement (★ A.6 + F 본격)."""
        # ★ A.6 Step B: dominance 검출
        dominant_types = _detect_dominant_types(self._last_encounter_types)

        # ★ F 본격: phase 결정 (★ Step A)
        hours = int(
            (game_context.get("v2_world_state") or {}).get(
                "hours_in_dungeon", 0
            )
        )
        phase = determine_phase(hours)
        ...
        for attempt in range(self.max_retry + 1):
            ...
            a6_violation, a6_reason = _is_violation(
                encounters, last_type, dominant_types
            )
            # ★ F commit 본격: phase mismatch check
            f_violation, f_reason = _is_phase_mismatch(encounters, phase)

            if not a6_violation and not f_violation:
            ...
            # 위반 — retry
            reason = a6_reason or f_reason
            if f_violation:
                self._phase_mismatch_count += 1
            log.warning(
                "GM rule violation turn=%d attempt=%d phase=%s reason=%s",
                turn_number,
                attempt + 1,
                phase.value,
                reason,
        ...
        # retry 모두 실패 → fallback narrative
        log.warning(
            "GM enforcement fallback turn=%d phase=%s violation=%s",
            turn_number,
            phase.value,
            last_violation,
        )
        ...
        if last_response is not None:
            ...
            return GMResponse(
                ...
                raw_text=f"(★ A.6+F fallback after {attempts} attempts)",
    ...
    def reset_history(self) -> None:
        """시뮬 시작 시 호출 (★ 직전 영향 X + metrics reset)."""
