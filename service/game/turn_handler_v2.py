...
def _defeat_boss(
    ...
    # ★ Phase 9.14 — 명성 ↑ (★ 보스 +10 + 균열 +5, party 전원 공유).
    # 본문 정합: 명성은 파티/본인 단위 — 본 commit minimal 본격 party 전원 동일 부여.
    fame_total = FAME_GAIN_BOSS_DEFEAT + FAME_GAIN_RIFT_CLEAR
    for member in party:
        if not member.is_alive():
            continue
        member.fame += fame_total
        side.append(
            f"fame_gain={member.name}:+{fame_total}_boss_rift"
        )
    if exp_awarded > 0:
        side.append(f"exp_gained={attacker.name}:{exp_awarded}")
...
# ─── Phase 9.14 fame system (★ 452화 명성 / 74화 '작은 발칸') ───
#
# 본문 정합:
# - 452화: 명성 → 초면 NPC 기본 호감도 ↑
# - 74-79화: 비요른 '작은 발칸' 명성 호칭
#
# 추측 수치 (본문 X — 후속 발견 시 보강):
# - 보스 처치 +10 / 균열 클리어 +5 (★ _defeat_boss site 단일 처리)
# - fame_bonus = fame // 10 (★ fame 100 → 호감도 +10)
FAME_GAIN_BOSS_DEFEAT: int = 10
FAME_GAIN_RIFT_CLEAR: int = 5
FAME_PER_AFFINITY_BONUS: int = 10
...
def execute_dialogue(
    ...
    # ★ Phase 9.14 — 초면 NPC fame bonus (★ 452화 정합).
    # 초면 = npc_affinities dict 본격 key 본격 X.
    is_first_meet = npc_id not in world.npc_affinities
    fame_bonus = 0
    if is_first_meet:
        fame_bonus = actor.fame // FAME_PER_AFFINITY_BONUS
        current = fame_bonus
    else:
        current = world.npc_affinities[npc_id]
    ...
    if is_first_meet and fame_bonus > 0:
        message += (
            f" (★ 명성 {actor.fame} 본격 초면 +{fame_bonus})"
        )
